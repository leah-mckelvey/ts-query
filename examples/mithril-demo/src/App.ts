    ...
    return m('div.app', [
      ...
      m('div.section', [
        ...
        m(UserQueryComponent, {
          children: (userQuery: QueryState<{ id: number; name: string; email: string }>) => [
            userQuery.isLoading && m('div.loading', 'Loading user...'),

            userQuery.isError && m('div.error', [
              'Error: ',
              userQuery.error?.message || 'Failed to fetch user',
            ]),

            userQuery.isSuccess && userQuery.data && m('div.success', [
              m('h3', 'User Data:'),
      ...
      m('div.section', [
        ...
        m(CreatePostMutationComponent, {
          children: (createPostMutation) => [
            ...
            createPostMutation.state.isError && m('div.error', [
              'Error: ',
              createPostMutation.state.error?.message || 'Failed to create post',
            ]),

            createPostMutation.state.isSuccess && createPostMutation.state.data && m('div.success', [
              m('h3', 'Post Created:'),
              m('p', [m('strong', 'ID: '), createPostMutation.state.data.id]),
              m('p', [m('strong', 'Title: '), createPostMutation.state.data.title]),
              m('p', [m('strong', 'Body: '), createPostMutation.state.data.body]),
            ]),
          ],
        }),
      ]),
